...
contract('ReserveAuction', async function(accounts) {
  let token, metadata
  ...
  before(done => {
    ;(async () => {
      try {
        ...
        token = await ReserveAuction.new("ReserveAuction", "TKN", metadata.address)
        var tx = await web3.eth.getTransactionReceipt(token.transactionHash)
        totalGas = totalGas.plus(tx.gasUsed)
        console.log(_ + tx.gasUsed + ' - Deploy ReserveAuction')
        token = await ReserveAuction.deployed()
  ...
  describe('ReserveAuction.sol', function() {
    it('should return metadata uints as strings', async function() {

      const URI = 'https://ReserveAuction.app/metadata/'
    ...
    it('should mint a token from the owner account', async function() {
      ...
      assert(
        ...
        "ReserveAuction at index 0 is " + tokenId.toString(10)
      )
    })
  })
})